	...
	if(isset($_POST['sub'])){
		...
		if($_FILES["img"]["type"]!=''){
			...
			if ((($_FILES["img"]["type"] == "image/gif")|| ($_FILES["img"]["type"] == "image/jpeg")|| ($_FILES["img"]["type"] == "image/pjpeg") || ($_FILES["img"]["type"] == "image/png"))){
		  		...
		  		echo "<script>alert('图片上传失败！');location.href='/zkadmin/editbannertel.php?id=".$id."';</script>";
		  	}
		  	if(isset($img)){
		  		$alt=$_POST['alt'];
		  		$url=$_POST['url'];
		  		$hot=$_POST['hot'];
		  		$sql="update dede_bannertel set img='$img',alt='$alt',url='$url',hot='$hot' where id=$id";
		  		mysqli_query($link,$sql);
		  		echo "<script>alert('修改成功！');location.href='/zkadmin/bannertel.php';</script>";
		  	}
		  }else{
		  	$alt=$_POST['alt'];
		  	$url=$_POST['url'];
		  	$id=$_POST['id'];
		  	$hot=$_POST['hot'];
		  		$sql="update dede_bannertel set alt='$alt',url='$url',hot='$hot' where id=$id";
		  		mysqli_query($link,$sql);
		  		echo "<script>alert('修改成功！');location.href='/zkadmin/bannertel.php';</script>";
		  }
...
<body>
	<form action="" method='post' enctype="multipart/form-data">
		<?php 
			$id=$_GET['id'];
			$sql="select * from dede_bannertel where id=$id";
