<?php 
	...
	if(isset($_POST['sub'])){
		...
		if ((($_FILES["img"]["type"] == "image/gif")|| ($_FILES["img"]["type"] == "image/jpeg")|| ($_FILES["img"]["type"] == "image/pjpeg") || ($_FILES["img"]["type"] == "image/png"))){
	  		...
	  		echo "<script>alert('图片上传失败！');location.href='/zkadmin/addbanner.php';</script>";
	  	}
	  	if(isset($img)){
	  		$alt=$_POST['alt'];
	  		$url=$_POST['url'];
	  		$hot=$_POST['hot'];
	  		$sql="insert into dede_banner values(null,'$img','$url','$alt','$hot')";
	  		mysqli_query($link,$sql);
	  		echo "<script>alert('添加成功！');location.href='/zkadmin/banner.php';</script>";
	  	}
	}
 ?>
 <html>
